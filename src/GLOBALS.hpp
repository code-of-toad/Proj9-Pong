...
/**
 * @brief Converts an sf::Keyboard::Key enum value to a human-readable string.
 *
 * This function maps an sf::Keyboard::Key enum value to its corresponding string representation.
 * If the key does not match any known value, it returns "Unknown".
 *
 * @param key The sf::Keyboard::Key value to convert.
 * @return A std::string representing the human-readable name of the key.
 *
 * Example usage:
 * @code
 * sf::Keyboard::Key key = sf::Keyboard::A;
 * std::string keyName = keyToString(key);
 * std::cout << "The key is: " << keyName << std::endl; // Output: "The key is: A"
 * @endcode
 */
inline std::string keyToString(const sf::Keyboard::Key key) {
    switch (key) {
